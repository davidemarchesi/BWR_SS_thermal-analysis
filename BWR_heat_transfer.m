...
T_max_min = max(Temp_wall1_ag) ;
T_max_max = max(Temp_wall4_kan);
alpha_max_min = max(alpha4_kan);
alpha_max_max = max(alpha1_ag) ;
...
tiledlayout(1,1)
[XX1,YY1] = meshgrid(m_ch_vec,q0_l_vec) ;
...
title('afnini')
ylabel('Linear Heat Peak q_0'' (kW/m)')
xlabel('Channel Mass Flow m_{ch} (kg/s)')
...
figure()
hold on
title('Groenveld LUT predicted CHF vs BWR heat flux axial profile')
